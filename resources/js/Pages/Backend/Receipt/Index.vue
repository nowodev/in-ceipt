    ...
    <app-layout title="Receipt">
        ...
        <CardLayout>
            <div class="p-6 text-right bg-white border-b border-gray-200">
                <ButtonLink :href="route('receipt.create')">Create</ButtonLink>
            </div>
                            ...
                            <table class="min-w-full divide-y divide-gray-200">
                                ...
                                <tbody class="bg-white divide-y divide-gray-200">
                                    <tr v-for="(receipt, index) in receipts.data" :key="receipt.id">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="text-sm text-gray-900">{{ receipt.customer.fullname }}</div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="text-sm text-gray-900">{{ receipt.serial_no }}</div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                        ...
                                        <td class="flex items-center px-6 py-4 space-x-4 text-sm font-medium whitespace-nowrap">
                                            ...
                                            <a href="#" @click="sendMail(receipt.id)">
                                            ...
                                            <Link :href="route('receipt.edit', receipt)" class="">
                                                <svg class="w-5 h-5" fill="currentColor"
                                            ...
                                            <a class="cursor-pointer" @click="confirmDelete(receipt)">
                                                <svg class="w-5 h-5" fill="currentColor"
                                                     viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                                                    <path clip-rule="evenodd"
            ...
            <DialogModal :show="confirmingDelete" @close="closeModal">
                ...
                <template #footer>
                    ...
                    <Button class="ml-3" @click="deleteReceipt(delete_data)">
                        Delete Receipt
                    </Button>
                </template>
            </DialogModal>
    ...
    export default defineComponent({
        ...
        methods: {
            ...
            sendMail: function (id) {
                this.$inertia.get(route('receipt.send_mail', id), {
                    preserveScroll: true,
                    preserveState: false
                })
            },
            ...
            deleteReceipt: function (receipt) {
                this.$inertia.delete(route('receipt.destroy', receipt), {
                    preserveScroll: true,
                    preserveState: false
                })
            }
